...
public class HBanner extends FrameLayout implements OnPageChangeListener {
    ...
    private int videoGravity = VideoGravityType.CENTER;
    private int imageGravity = ImageGravityType.CENTER;
    ...
    //设置pager背景
    public HBanner setPageBackground(Drawable background) {
        if (viewPager != null)
            viewPager.setBackground(background);
        return this;
    }

    public HBanner setPageBackgroundResource(int resource) {
        if (viewPager != null)
            viewPager.setBackgroundResource(resource);
        return this;
    }

    public HBanner setPageBackgroundColor(int color) {
        if (viewPager != null)
    ...
    @IntDef({VideoGravityType.CENTER,
            VideoGravityType.CENTER_HORIZONTAL,
            VideoGravityType.FULL_SCREEN})
    @Retention(RetentionPolicy.SOURCE)
    private @interface subVideoGravity {
    }

    @IntDef({ImageGravityType.MATRIX,
    ...
    @Retention(RetentionPolicy.SOURCE)
    private @interface subImageGravity {
    }

    //子视频显示位置
    public HBanner setVideoGravity(@subVideoGravity int gravity) {
        this.videoGravity = gravity;
        return this;
    }

    //子图片显示位置
    public HBanner setImageGravity(@subImageGravity int gravity) {
        ...
        if (list.size() != 0) {
            for (ViewItemBean bean : list) {
                if (bean.getUrl() instanceof Uri) {
                    ...
                    LogUtils.i("lake", "checkCache: " + cacheFilePath + type);
        ...
        HttpThreadPool.getInstance().post(() -> {
            ...
            HttpClient.getInstance().Get(httpParam, new HttpCallback.ProgressRequestHttpCallback<File>() {
                ...
                public void success(File result) {
                    LogUtils.i("lake", "success: " + result.getName());
                }

                @Override
                public void failed(String Msg) {
                    LogUtils.e("lake", "failed: " + Msg);
                    if (file.exists())
                        file.delete();
                }
                ...
                public void progress(float progress, float count) {
                    ...
                    LogUtils.i("lake", "progress: " + String.format("%.2f", percent) + "%");
                }
            });
        });
    }
    ...
    private void setViewByLoader(ViewItemBean viewItemBean) {
        ...
        if (isVideo) {
            if (videoLoader != null)
                v = videoLoader.createView(context, videoGravity);
        } else {
            if (imageLoader != null)
                v = imageLoader.createView(context, imageGravity);
        }
        if (v == null) {
    ...
    private void startAutoPlay() {
        ...
        LogUtils.i("auto", "startAutoPlay: " + delayTime);
    ...
    private final Runnable task = new Runnable() {
        ...
        public void run() {
            if (count > 1 && isAutoPlay) {
                ...
                if (currentItem == 1) {//最后一张或者第一张
                    viewPager.setCurrentItem(currentItem, false);
                } else {
                    viewPager.setCurrentItem(currentItem);
                }
                int delayTime = subList.get(currentItem).getTime();
                LogUtils.i("auto", "currentItem: " + currentItem + ",delayTime=" + delayTime);
                changeTime = System.currentTimeMillis() + delayTime;
                handler.postDelayed(task, delayTime);
            } else {
    ...
    class BannerPagerAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, final int position) {
            LogUtils.i("lake", "instantiateItem: " + position);
        ...
        public void destroyItem(ViewGroup container, int position, Object object) {
            LogUtils.i("lake", "destroyItem: " + position);
            View item = (View) object;
            if (item instanceof ViewGroup) {
                ViewGroup viewGroup = (ViewGroup) item;
                if (viewGroup.getChildCount() > 0 && viewGroup.getChildAt(0) instanceof VideoView) {
                    VideoView videoView = (VideoView) viewGroup.getChildAt(0);
                    videoLoader.onDestroy(videoView);
    ...
    public void onPageSelected(int position) {
        LogUtils.i("auto", "onPageSelected: " + position);
